...
RECCOBEATS_CACHE_PATH = (
    Path(__file__).parent.parent.parent.parent / "data" / "lib_reccobeats_cache.csv"
)
...
def _load_cache() -> dict[str, dict]:
    ...
    with open(RECCOBEATS_CACHE_PATH, newline="", encoding="utf-8") as f:
        ...
        return {
            row["spotify_id"]: {
                k: _coerce(v) for k, v in row.items() if k != "spotify_id"
            }
            for row in reader
        }
    ...
    for i in range(0, len(missing), 40):
        ...
        try:
            ...
            if "content" not in data:
                print(
                    f"  Warning: reccobeats response missing 'content' for chunk {i // 100 + 1}: {data}"
                )
                continue
            ...
            print(
                f"  Warning: reccobeats request failed for chunk {i // 100 + 1}: {exc}"
            )
        except (KeyError, ValueError) as exc:
            print(
                f"  Warning: reccobeats response parse error for chunk {i // 100 + 1}: {exc}"
            )
