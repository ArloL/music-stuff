...
public interface SongRepository extends CrudRepository<Song, Long> {
	...
	@Query(
		"""
				SELECT * FROM SONGS s
				WHERE
				ID  IN (SELECT PLAYLISTITEMS_ID FROM PLAYLISTS_SONGS ps LEFT JOIN PLAYLISTS p ON p.ID = ps.PLAYLISTS_ID WHERE p.NAME = 'Critical Mass Selection')
				AND GENRE IN ('Electronic', 'Ambient')
				AND RATING >= 80
				AND RATINGCOMPUTED = FALSE
				AND COMMENTS != 'ignore'
	...
	@Query(
		"""
				...
				WHERE
				ID NOT IN (SELECT PLAYLISTITEMS_ID FROM PLAYLISTS_SONGS ps LEFT JOIN PLAYLISTS p ON p.ID = ps.PLAYLISTS_ID WHERE p.NAME = 'Critical Mass Played')
				AND ID IN (SELECT PLAYLISTITEMS_ID FROM PLAYLISTS_SONGS ps LEFT JOIN PLAYLISTS p ON p.ID = ps.PLAYLISTS_ID WHERE p.NAME = 'Critical Mass Selection')
				AND GENRE IN ('Electronic', 'Ambient')
				AND RATING >= 80
				AND RATINGCOMPUTED = FALSE
				AND COMMENTS != 'ignore'
