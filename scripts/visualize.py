...
def create_plotly_network(G, layout_type='spring'):
        ...
        for i, community in enumerate(communities):
            ...
            if len(community) > 1:
                ...
                for node in community:
                    pos[node] = (center_x + sub_pos[node][0], center_y + sub_pos[node][1])
            else:
    ...
    for node in G.nodes():
        ...
        node_info.append(f"{node}<br>Key: {key}<br>BPM: {bpm}<br>Connections: {connections}")
    ...
    html.Div([
        ...
        dcc.RadioItems(
            ...
            options=[
                ...
                {'label': 'Force Directed + Communities', 'value': 'force_directed'},
